...
describe.sequential("issue thread interaction routes", () => {
  ...
  it("rejects remembered tool permission before replaying an accepted secret binding", async () => {
    mockInteractionService.getForIssue.mockResolvedValueOnce({
    ...
    const approveSecretProposal = vi.fn();
    const app = await createApp(undefined, { approveSecretProposal });
    ...
    expect(res.status).toBe(422);
    expect(res.body.error).toBe("Remembered permission is only supported for tool reviews");
