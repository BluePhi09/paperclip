...
describe("Kubernetes Exec PTY transport", () => {
  it("uses tty=true and leaves stdin open for delayed secret input", async () => {
    ...
    const connection = await connectKubernetesLoginPty(scope, ["claude"], { tty: true, output, exit }, {} as never);
  ...
  it("resolves a not-yet-created pod before opening Exec", async () => {
  ...
  it("closes the socket when buffered stdin exceeds the host-side cap", async () => {
    const exit = vi.fn();
    const connection = await connectKubernetesLoginPty(scope, ["claude"], { tty: true, output: vi.fn(), exit }, {} as never);
  ...
  it("reports abnormal socket close as null exit", async () => {
    const exit = vi.fn();
    await connectKubernetesLoginPty(scope, ["claude"], { tty: true, output: vi.fn(), exit }, {} as never);
