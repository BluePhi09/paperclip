...
A weekly schedule (Mondays) also runs in the same workflow, but it never builds or publishes anything: it queries the npm registry for the current `latest` version of each harness's CLI package and opens/updates a tracking issue (labeled `agent-runtime-cli-check`) so a maintainer can decide whether a rebuild is warranted. Actually publishing a refreshed image still always requires an explicit `push` or `workflow_dispatch` run.
