...
A weekly schedule (Mondays) also runs in the same workflow, but it never builds or publishes anything: it queries the npm registry for the current `latest` version of each harness's CLI package, pulls the currently published `:latest` image and probes its baked-in CLI version with `--version` (read-only; no build/sign/push), and opens/updates a tracking issue (labeled `agent-runtime-cli-check`) flagging any harness where the deployed `:latest` image's version differs from npm's current latest. Actually publishing a refreshed image still always requires an explicit `push` or `workflow_dispatch` run.
