...
This suite can spend provider money, expose selected local API credentials to isolated
...
`DAYTONA_API_KEY` in it. `KIMI_MODEL_API_KEY` and `XAI_API_KEY` are local-only
catalog names for pending explicit profiles; they are intentionally absent from
the workflow environment. Do not duplicate these credentials as repository- or
