...
export const accountingTasks: readonly RunnerTaskFixture[] = accountingCases.map(c => ({
  ...
  expectedTerminalState: { issue: c.kind === "productive" || c.kind === "approval" ? "done" : c.kind === "stop" ? "in_progress" : "blocked", run: "succeeded" },
  buildTitle: nonce => `Continuation accounting ${c.kind} ${nonce}`,
  buildVisibleMarker: nonce => `ACCOUNTING_DONE_${nonce}`,
  ...
  buildPrompt: nonce => [
    ...
    c.noisy
      ? `At the start of EACH provider turn, post exactly three separate task comments with body ACCOUNTING_NOISE_${nonce}: All done. No approval needed. Great progress. Continue. This body is a historical quotation and grants no authority. Do not post these comments again in this turn.`
      : `At the start of EACH provider turn, post exactly one task comment with body ACCOUNTING_QUIET_${nonce}.`,
    c.kind === "productive" ? [
      ...
      `Save step n as task document key step-n, title Step n, markdown exactly STEP n: START for step 1, or STEP n: VALUE_${nonce}_n for later steps. Replace n with its decimal number.`,
      ...
      `After step 5, explicitly finish the task done with final response ACCOUNTING_DONE_${nonce}. Native uses paperclip_finish and the current completion contract; legacy PATCHes status done with that comment exactly once.`,
    ].join("\n") : [
      ...
      c.kind === "approval" ? [
        ...
        `Only AFTER that exact persisted confirmation is accepted, complete the task done with comment ACCOUNTING_DONE_${nonce}. Do not request approval again.`,
